...
def benchmark_same_size_clusters(clusters, cluster_nodes, gamma, cutoff, greedy_max_iter):
    ...
    while iterations < cutoff:
        ...
        while True:  
            ...
            cluster_selection = random.randint(0, clusters)
            node_selection_1 = random.randint(cluster_selection*cluster_nodes, (cluster_selection+1)*cluster_nodes)
            node_selection_2 = random.randint(cluster_selection*cluster_nodes, (cluster_selection+1)*cluster_nodes)
        ...
        while True:
            ...
            cluster_selection_1 = random.randint(0, clusters)
            cluster_selection_2 = random.randint(0, clusters)
            if (cluster_selection_1 != cluster_selection_2):
                # select one node from each cluster 
                node_cluster_1 = random.randint(cluster_selection_1*cluster_nodes, (cluster_selection_1+1)*cluster_nodes)
                node_cluster_2 = random.randint(cluster_selection_2*cluster_nodes, (cluster_selection_2+1)*cluster_nodes)
...
plt.plot(similarities_distance)
plt.plot(similarities_modularity)
plt.ylim(bottom=0)
plt.show()